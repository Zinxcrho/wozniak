...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.db.data.users[m.sender].healt > 79) {
            ...
            let exp = (Math.floor(Math.random() * 20000) + (kuda * 7000))
            let uang = `${Math.floor(Math.random() * 30000)}`.trim() 
            let _potion = `${Math.floor(Math.random() * 102)}`.trim()
            let string = `${Math.floor(Math.random() * 1532)}`.trim()
            let kayu = `${Math.floor(Math.random() * 1569)}`.trim()
